...
class WikiDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        tokenizer = RobertaTokenizer.from_pretrained(self.tokenizer_name_or_path, use_fast=True)
        ds = load_dataset('wikitext', 'wikitext-103-raw-v1')  # , split='train[:5%]').train_test_split(.1)
        ds = ds.filter(lambda example: example['text'] != '' and not example['text'].startswith(' ='))
        ds = ds.map(lambda ex: tokenizer(ex['text'], padding=True, truncation=True, max_length=self.max_length), batched=True)
    ...
    def val_dataloader(self):
        return DataLoader(self.ds['validation'], batch_size=self.batch_size, num_workers=self.num_workers, collate_fn=self.collate_fn)
